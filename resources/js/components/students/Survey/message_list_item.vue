...
function positiveVote() {
    axios.post(`/students/survey/remark/${props.remark.id}/vote`, { type: 'like', code: props.code })
        ...
        .catch(error => {
            console.error('Erreur lors du like de la remarque :', error?.response?.data?.message || error);
            err.value = error?.response?.data?.message || 'Une erreur est survenue';
        });
}
...
function negativeVote() {
    axios.post(`/students/survey/remark/${props.remark.id}/vote`, { type: 'dislike', code: props.code })
        ...
        .catch(error => {
            console.error('Erreur lors du dislike de la remarque :', error?.response?.data?.message || error);
            err.value = error?.response?.data?.message || 'Une erreur est survenue';
        });
}
</script>
        ...
        <div class="flex items-center gap-2 ml-4">
            <template v-if="props.remark.ip_address !== props.ip">
                <button @click="positiveVote"
                    :class="[props.vote?.type === 'like' || vote === 'like' ? 'bg-green-500 scale-110' : 'bg-gray-100 hover:bg-green-100', 'p-2 rounded-lg transition-all active:scale-95']">
                    <img src="/icons/like.png" :class="props.vote?.type === 'like' || vote === 'like' ? 'brightness-0 invert' : ''"
                        class="h-5 w-5" alt="Like">
                </button>

                <button @click="negativeVote"
                    :class="[props.vote?.type === 'dislike' || vote === 'dislike' ? 'bg-red-500 scale-110' : 'bg-gray-100 hover:bg-red-100', 'p-2 rounded-lg transition-all active:scale-95']">
                    <img src="/icons/dislike.png" :class="props.vote?.type === 'dislike' || vote === 'dislike' ? 'brightness-0 invert' : ''"
                        class="h-5 w-5" alt="Dislike">
                </button>
            </template>
