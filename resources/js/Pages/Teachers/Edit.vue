...
onMounted(() => {
    const surveyId = window.location.pathname.split('/').slice(-2)[0];
    axios.get(`/teachers/survey/${surveyId}`)
        .then(response => {
            survey.value = response.data;
            console.log('Sondage récupéré pour modification:', survey.value);
        })
        .catch(error => {
...
function submit() {
    ...
    axios.patch(`/teachers/survey/${survey.value.id}`, survey.value)
        .then(response => {
            console.log('Sondage mis à jour avec succès:', response.data);
            window.location.href = '/teachers/home';
        })
        .catch(error => {
            console.error('Erreur lors de la mise à jour du sondage:', error);
            errorMsg.value = error.response.data.message || 'Une erreur est survenue lors de la mise à jour du sondage.';
        });
}
</script>
...
<template>
    ...
    <AppLayout>
        <div>
            <h2 class="text-2xl md:text-3xl font-extrabold text-gray-900 tracking-tight">Espace enseignant - Modification</h2>
